...
import com.ibm.complaint.repository.ComplaintMongoRepository;
...
public class ComplaintServiceImpl implements ComplaintService {
	...
	@Autowired
	private ComplaintMongoRepository complaintrepo;
	...
	public String registerComplaint(Complaint complaint) {
		...
		Complaint obj = (Complaint) complaintrepo.save(complaint);
		return obj.getId();
	}
	...
	public Complaint getComplaintDetails(String id) throws NoComplaintsFoundException {
		Complaint comList;
		/*
		 * try { comList = repo.getComplaintDetails(id); } catch (IOException e) { throw
		 * new NoComplaintsFoundException("Error occurred due to:" + e.getMessage()); }
		 * return comList;
		 */
		comList = complaintrepo.findBy_id(id);
		return comList;
	}
